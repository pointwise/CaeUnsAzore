...
struct AZORE_DATA {
    ...
    PWP_UINT32          vcCellType{ AZORE_CELL_MIXED };
...
convertCellType(const PWGM_ENUM_ELEMTYPE pwType)
{
    ...
    case PWGM_ELEMTYPE_TRI:
        return AZORE_CELL_TRI;
        break;
    case PWGM_ELEMTYPE_TET:
        return AZORE_CELL_TET;
        break;
    case PWGM_ELEMTYPE_QUAD:
        return AZORE_CELL_QUAD;
        break;
    case PWGM_ELEMTYPE_HEX:
        return AZORE_CELL_HEX;
        break;
    case PWGM_ELEMTYPE_PYRAMID:
        return AZORE_CELL_PYR;
        break;
    case PWGM_ELEMTYPE_WEDGE:
        return AZORE_CELL_WEDGE;
        break;
    default:
        break;
    }
    return AZORE_CELL_OTHER;
}
...
{
    PWP_UINT32 result = AZORE_FACE_OTHER;
    switch (pwType) {
    case AZORE_CELL_TRI:
    case AZORE_CELL_TET:
        result = AZORE_FACE_TRI;
        break;
    case AZORE_CELL_QUAD:
    case AZORE_CELL_HEX:
        result = AZORE_FACE_QUAD;
        break;
    case AZORE_CELL_WEDGE:
    case AZORE_CELL_PYR:
    case AZORE_CELL_MIXED:
        result = AZORE_FACE_MIXED;
        break;
    case AZORE_CELL_OTHER:
        result = AZORE_FACE_OTHER;
        break;
    default:
        result = AZORE_FACE_OTHER;
        break;
    ...
    if (is2D && (AZORE_FACE_TRI == result || AZORE_FACE_QUAD == result)) {
        result = AZORE_FACE_BAR;
    }
...
{
    PWP_UINT32 cellCode = AZORE_CELL_OTHER;
    PWP_BOOL result = PWP_TRUE;

    if (isMixedBlock(elemCnts)) {
        cellCode = AZORE_CELL_MIXED;
    } 
    else {
...
{
    // For Azore compatibility, change to lowercase and replace spaces with 
    // spaceReplacement.
...
{
    fprintf(rti.fp, "(%d \"", AZORE_COMMENT);
    va_list args;
    va_start(args, format);
...
{
    writeSectionListHdrNoCR(rti, AZORE_FACES, "%x %x %x %x %x", rti.data->zone,
...
writeOneFace(const CAEP_RTITEM &rti, const PWGM_FACESTREAM_DATA &face)
{
    ...
    if (AZORE_CELL_MIXED == rti.data->vcCellType) {
...
// Write the nodes section AZORE_NODES(10)
static bool
...
{
    // Make zone names safe for Azore write
    makeSafe(condType, '-');
    makeSafe(condName, '_');
    // (45 (3 interior interior-3)())
    writeSectionLine(rti, AZORE_ZONE, "%d %s %s", rti.data->zone,
...
writeCloseFaceZone(CAEP_RTITEM &rti, const PWGM_ENUM_FACETYPE faceType)
{
    ...
    case PWGM_FACETYPE_CONNECTION:
    case PWGM_FACETYPE_INTERIOR: {
        writeFacesListHdr(rti, AZORE_INTERIOR);
        pwpFileSetpos(rti.fp, &eof);
...
processBlockVCMap(CAEP_RTITEM &rti)
{
    ...
    for (PWP_UINT32 blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
        ...
        PWP_UINT32 elemTypes = AZORE_FACE_OTHER;
        ...
        if (rti.data->blockVCMap.end() == mIter) {
            ...
            if (grpStats->elemTypes != elemTypes) {
                grpStats->elemTypes = AZORE_FACE_MIXED;
            }
...
// Write the Cell zone section AZORE_CELLS(12)
...
writeVCZone(CAEP_RTITEM &rti, const PWP_UINT32 &vcId)
{
    ...
    if (blockToVCs.end() != mIter && !CAEPU_RT_IS_ABORTED(&rti)) {
        ...
        // Write Azore Cell line
        fprintf(rti.fp, "(%d (%x %x %x 1 %x", AZORE_CELLS, rti.data->zone, 
            rti.data->blockIndex,
            rti.data->blockIndex + grpStats->groupBlkCells - 1,
            grpStats->elemTypes);
    ...
    if (!rti.data->headerOpen && currentVCId != rti.data->prevVCId) {
        ...
        if (PWGM_FACETYPE_CONNECTION != face->type) {
            ...
            if (CAEPU_RT_DIM_3D(&rti)) {
                ...
                rti.data->vcCellType = AZORE_FACE_MIXED;
            }
        }
    ...
    const CAEP_WRITEINFO *pWriteInfo)
{
    ...
    if (pRti && pRti->fp && model && pWriteInfo) {
        ...
        AZORE_DATA AzoreData;
        pRti->data = &AzoreData; // cppcheck-suppress autoVariables
